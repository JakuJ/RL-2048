...
Ensemble::Ensemble(double lr) : learning_rate(lr) {
    ...
    for (auto &&t : SymmetryExpander::expand<7>(m, shape))
        tuples.push_back(std::move(t));

    std::tuple<int, int> other_shape[7] = {
            {1, 3},
            {1, 2},
            {1, 1},
            {1, 0},
    ...
    for (auto &&t : SymmetryExpander::expand<7>(m, other_shape))
        tuples.push_back(std::move(t));
}
